        ...
        >
        {
            data.type === 'Videos' ? (
                <video 
                    ...
                    className={`transition-all ease-in-out duration-500 ${isEditMode ? "" : "group-hover:-translate-y-2"} rounded-2xl w-full h-full object-cover`}
                >
                    <source src={data.thumbnail} type="video/mp4" />
                </video>
            ) : (
                <div className={`transition-all ease-in-out duration-500 ${isEditMode ? "" : "group-hover:-translate-y-2"} w-full h-full`}>
                    <img
                        src={data.thumbnail}
                        alt={data.altText}
                        onClick={() => redirectToCollection(data.id)}
            ...
            <div className="flex justify-between relative px-4 py-2">
                ...
                {isEditMode ? <></> : (
                    <div className="transition-all ease-in-out duration-500 absolute right-5 opacity-100 sm:opacity-0 sm:group-hover:opacity-100">
