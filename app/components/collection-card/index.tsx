...
export default function CollectionCardComponent({ data }: CollectionCardProps) {
    ...
    return (
        <div className="flex flex-col gap-y-3 cursor-pointer transition-all ease-in-out duration-300 group">
        {
            data.type === 'Videos' ? (
                <video onClick={() => redirectToCollection(data.id)} className="rounded-2xl w-full h-full">
                    <source src={data.thumbnail} type="video/mp4" />
                </video>
            ) : (
                <div className="w-full group-hover:-translate-y-2">
                    <img
                        src={data.thumbnail}
                        alt={data.altText}
            ...
            <div className="flex justify-between relative px-4 py-2">
                <div className="flex flex-col gap-y-1">
                    ...
                    <p className="text-sm text-subtext font-regular">{`${data.noOfItems} ${data.type}`}</p>
                </div>
