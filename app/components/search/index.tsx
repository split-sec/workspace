        ...
        <div className="outline outline-1 outline-inactive-button-border rounded-md p-2 md:p-3 flex items-center justify-between grow">
            <div className="flex items-center gap-x-2">
                ...
                <input 
                    ...
                    onChange={(event: React.FormEvent<HTMLInputElement>) => handleChange(event.currentTarget.value)}
                    className="bg-bright-bg text-[10px] md:text-xs text-primary font-regular grow cursor-pointer"
                />
            </div>
            <img 
                src="/close-icon.svg" 
                className={`h-[14px] w-[14px] cursor-pointer ${inputValue.length > 0 ? "block" : "hidden"}`} 
                alt="Delete Icon" 
